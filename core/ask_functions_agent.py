...
def initialize_similarity_dictionaries(use_db: bool = True,
                                       use_neo4j: bool = True) -> None:
    """
    ...
    if use_neo4j:
        try:
            ...
            print(
                f"Failed to load from Neo4j: {e}, falling back to PostgreSQL")
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        ...
        for table_name in col_name_mapping_dict:
            ...
            futures.append(
                executor.submit(_process_table_attributes, table_name,
                                'fclass')
            )
...
def _process_table_attributes(table_name: str, attribute_type: str) -> Tuple[
    str, str, Set[str]]:
    """
...
def limit_total_words(text_tuple: Tuple[str, ...], max_length: int = 10000) -> \
List[str]:
    """
...
class SimilarityCalculator:
    ...
    @cached(lambda self, string,
                   lst: f"partial_match:{string}:{hash(tuple(sorted(lst)))}")
...
class FeatureMatcher:
    ...
    def process_feature_matching(
        ...
        """
        try:
            query_feature = query_feature.strip() if isinstance(query_feature,
                                                                str) else str(
                query_feature)
        except Exception as e:
        ...
        query_list = query_feature.split(
            " and ") if " and " in query_feature else [query_feature]
    ...
    def _clean_query(self, query: str, table_name: str, col_name: str) -> str:
        ...
        """
        stop_words = ['named', 'is', 'which', 'where', 'has', 'call', 'called',
                      table_name, col_name]
...
class GeographicProcessor:
    ...
    @cached()
    def judge_geo_relation(self, query: str) -> Optional[
        ...
        if distance_match and any(
                word in query_clean for word in ['around', 'near', 'close']):
    ...
    def _process_complex_geo_relation(self, query: str) -> Optional[
        Dict[str, Union[str, int]]]:
        """
        ...
        try:
            ...
            if 'geo_calculations' in json_result and \
                    json_result['geo_calculations']['exist']:
    ...
    def process_geographic_filter(
        ...
        if isinstance(id_list_subject, str):
            id_list_subject = self._get_entity_id_list(id_list_subject,
                                                       bounding_box)

        if isinstance(id_list_object, str):
            id_list_object = self._get_entity_id_list(id_list_object,
                                                      bounding_box)

        # Check for negation
        versa_sign = self._find_negation(query)
        if versa_sign:
            query = re.sub(r'\b(not|no|never|none)\b', '', query,
        ...
        try:
            ...
            if 'subject' in geo_result and 'id_list' in geo_result['subject']:
                target_labels = self._get_labels_from_ids(
                    geo_result['subject']['id_list'])
    ...
    def _get_entity_id_list(self, entity: str,
                            bounding_box: Optional[Dict] = None) -> Dict:
        """
    ...
    for d in dict_list:
        ...
        for key, subdict in d.items():
            if key not in result:
                result[key] = subdict.copy() if isinstance(subdict,
                                                           dict) else subdict
            else:
...
class EntityProcessor:
    ...
    def judge_table_name(self, query: str) -> Optional[Dict[str, str]]:
        ...
        soil_keywords = ['planting', 'potatoes', 'tomatoes', 'strawberr',
    ...
    def process_entity_list(
        ...
        if table_restriction:
            safe_print(
                f"Table restriction detected: {table_restriction['database']}")
    ...
    @cached()
    def _merge_id_lists(self, id_list_collection: List[Dict]) -> Dict[
        str, Any]:
        """
...
class BoundingBoxManager:
    ...
    def set_bounding_box(self, region_name: str,
                         query: Optional[str] = None) -> Dict[str, Any]:
        """
        Set bounding box for the current session

        Note: This method modifies session state and should NOT be cached
        """
        if not region_name:
            # Clear bounding box
            session['globals_dict'] = None
            session['user_set_bounding_box'] = False  # Mark as not user-set
        ...
        try:
            ...
            session[
                'user_set_bounding_box'] = True  # Mark that user explicitly set this
            session.modified = True

            # Debug logging
            print(f"\n[SET_BOUNDING_BOX] Updated session:")
            print(f"  Region: {region_name}")
            print(f"  Session ID: {session.get('sid', 'unknown')}")
            print(f"  User set flag: {session.get('user_set_bounding_box')}")
...
def id_list_of_entity(
    ...
    """
    try:
        return entity_processor.process_entity_list(query, verbose,
                                                    bounding_box)
    except Exception as e:
...
def set_bounding_box(region_name: str, query: Optional[str] = None) -> Dict[
    str, Any]:
    """
...
def clear_all_caches() -> None:
    ...
    for obj in [query_processor, similarity_calc, feature_matcher,
                geo_processor, entity_processor, bbox_manager]:
    ...
    try:
        with driver.session() as neo4j_session:
            ...
            for rel_type in relationships[
                            :10]:  # Limit to first 10 relationship types
                rel_query = f"""
...
Then explain the results in natural language."""
        ...
        cypher_match = re.search(r'```cypher\s*(.*?)\s*```', response,
                                 re.DOTALL)
        ...
        if cypher_match:
            ...
            try:
                            ...
                            if value is not None:
                                if isinstance(value, (str, int, float, bool)):
                                    ...
                                    formatted_record[key] = str(value)[
                                                            :100]  # Truncate long values
                        result_summary += f"{i + 1}. {formatted_record}\n"

                # Generate natural language answer based on results
                answer_messages = [
                    message_template('system',
                                     'Based on the Cypher query results, provide a clear and concise answer to the user\'s question. Be specific about the data found.'),
                ...
                messages.append(
                    message_template('assistant', formatted_response))
                ...
                error_messages = [
                    message_template('system',
                                     'The Cypher query failed. Analyze the error and provide a helpful response to the user.'),
                    message_template('user',
                                     f"Query: {query}\nCypher: {cypher_query}\nError: {error_msg}")
                ]
